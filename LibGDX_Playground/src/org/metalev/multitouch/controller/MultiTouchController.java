...
public class MultiTouchController<T>
{
    static class Log
    {
        ...
        {
            Gdx.app.log("ERROR: ", string1 + " =\\\\\\= " + string2 + " -///- "
                    + e.getMessage());
        }

        public static void i(String string1, String string2)
        {
            Gdx.app.log("INFO: ", string1 + " =\\\\\\= " + string2);
        }
    }
    ...
    public MultiTouchController(boolean b,
            MultiTouchObjectCanvas<T> objectCanvas)
    {
        this(b, objectCanvas, true);
    }
    ...
    static
    {
        ...
        // }
        // multiTouchSupported = Gdx.input.supportsMultitouch();
    ...
    private static final MotionEvent[] bufferMoveEvents = new MotionEvent[MAX_TOUCH_POINTS];
    private int bufferMoveEvents_index = -1;

    /** Process incoming touch events */
    public boolean onTouchEvent(MotionEvent event)
    {
        if (DEBUG && event.action == MotionEvent.ACTION_UP)
            Log.i("MultiTouch", "Received UUUUUUUUP !!!!");
        if (event instanceof MotionEventBatch)
        {
            if (DEBUG)
                Log.i("MultiTouch MotionEventBatch",
            ...
            {
                if (DEBUG)
                    Log.i("MultiTouch", "NOT multiTouchSupported");
            }
            else
                if (event.action == MotionEvent.ACTION_MOVE)
                {
                    if (DEBUG)
                        Log.i("MultiTouch ACTION", "MotionEvent.ACTION_" + event.action );
                    if (bufferMoveEvents_index == -1)
                    {
                        bufferMoveEvents_index++;
                        bufferMoveEvents[bufferMoveEvents_index] = event;
                        return true;
                    }
                    boolean pointerAlreadyBuffered = false;
                    for (int i = 0; i <= bufferMoveEvents_index; i++)
                    {
                        MotionEvent ev = bufferMoveEvents[i];
                        if (event.pointer == ev.pointer)
                        {
                            pointerAlreadyBuffered = true;
                            break;
                        }
                    }
                    if (pointerAlreadyBuffered)
                    {
                        MotionEventBatch eventBatch = MotionEventBatch.create(
                                MotionEvent.ACTION_MOVE, bufferMoveEvents,
                                bufferMoveEvents_index);
                        if (DEBUG)
                            Log.i("MultiTouch BATCH 1", ""
                                    + bufferMoveEvents_index);
                        for (int i = 0; i < bufferMoveEvents.length; i++)
                        {
                            bufferMoveEvents[i] = null;
                        }
                        bufferMoveEvents_index = 0;
                        bufferMoveEvents[bufferMoveEvents_index] = event;
                        onTouchEvent(eventBatch);
                        return true;
                    }
                    else
                    {
                ...
                {
                    if (DEBUG)
                        Log.i("MultiTouch ACTION", "MotionEvent.ACTION_OTHER_"
                                + event.action);
                    if (bufferMoveEvents_index != -1)
                    {
                        MotionEventBatch eventBatch = MotionEventBatch.create(
                                ...
                                bufferMoveEvents_index);
                        if (DEBUG)
                            Log.i("MultiTouch BATCH 2", ""
                                    + bufferMoveEvents_index);
                        for (int i = 0; i < bufferMoveEvents.length; i++)
                        {
                            bufferMoveEvents[i] = null;
                        }
                        bufferMoveEvents_index = -1;
                        onTouchEvent(eventBatch);
                    }
                }
        try
        {
            // int pointerCount = multiTouchSupported ? (Integer)
            // m_getPointerCount.invoke(event) : 1;
            int pointerCount = multiTouchSupported ? (Integer) event
                    .getPointerCount() : 1;
            if (DEBUG)
            ...
            for (int histIdx = 0; histIdx <= histLen; histIdx++)
            {
                ...
                else
                {
                    ...
                    for (int ptrIdx = 0; ptrIdx < numPointers; ptrIdx++)
                    {
                        ...
                        // ptrIdx);
                        int ptrId = (Integer) event.getPointerId(ptrIdx);
                        ...
                        xVals[ptrIdx] = (Float) (processingHist ? event
                                .getHistoricalX(ptrIdx, histIdx) : event
                                .getX(ptrIdx));
                        // yVals[ptrIdx] = (Float) (processingHist ?
                        // m_getHistoricalY.invoke(event, ptrIdx, histIdx) :
                        // m_getY.invoke(event, ptrIdx));
                        yVals[ptrIdx] = (Float) (processingHist ? event
                                .getHistoricalY(ptrIdx, histIdx) : event
                                .getY(ptrIdx));
                        // pressureVals[ptrIdx] = (Float) (processingHist ?
                        // m_getHistoricalPressure.invoke(event, ptrIdx,
                        // histIdx) : m_getPressure.invoke(event, ptrIdx));
                        pressureVals[ptrIdx] = (Float) (processingHist ? event
                                .getHistoricalPressure(ptrIdx, histIdx) : event
                                .getPressure(ptrIdx));
                    }
                }
                // Decode event
                decodeTouchEvent(pointerCount, xVals, yVals, pressureVals,
                        ...
                        // && (action & ((1 << ACTION_POINTER_INDEX_SHIFT) - 1))
                        // != ACTION_POINTER_UP //
        ...
        switch (mMode)
        {
            ...
            case MODE_DRAG:
                ...
                if (!mCurrPt.isDown())
                {
                    ...
                    {
                        // Point 1 was already down and point 2 was just placed
                        // down
                        mMode = MODE_PINCH;
                        // Restart the drag with the new drag position (that is
                        // at the midpoint between the touchpoints)
                        anchorAtThisPositionAndScale();
                        // Need to let events settle before moving things, to
                        // help with event noise on touchdown
                    ...
                    {
                        // Point 1 is still down and point 2 did not change
                        // state, just do single-point drag to new location
                        if (mCurrPt.getEventTime() < mSettleEndTime)
                        {
                            // Ignore the first few events if we just stopped
                            // stretching, because if finger 2 was kept down
                            // while
            ...
            case MODE_PINCH:
                ...
                if (!mCurrPt.isMultiTouch() || !mCurrPt.isDown())
                {
                    ...
                    if (!mCurrPt.isDown())
                    {
                        ...
                        mMode = MODE_NOTHING;
                        objectCanvas.selectObject((selectedObject = null),
                                mCurrPt);
                    }
                    else
                    {
    ...
    public static interface MultiTouchObjectCanvas<T>
    {
         ...
         *        topmost object under this point, or if dragging/stretching the
         *        whole canvas, just return a reference to the canvas.
         ...
         *        Output parameter: You need to call objPosAndScaleOut.set() to
         *        record the current position and scale of obj.
         */
         ...
         *        information and utilities to calculate and cache multitouch
         *        pinch diameter etc. (Note: touchPoint is volatile, if you want
         *        to keep any fields of touchPoint, you must copy them before
         *        the method body exits.)
    ...
    public static class MotionEventBatch extends MotionEvent
    {
        MotionEvent[] array;
        int lastIndex;

        public static MotionEventBatch create(int action, MotionEvent[] array,
                int lastIndex)
        {
            MotionEventBatch event = new MotionEventBatch();
            event.array = array.clone();
        ...
        public float getHistoricalPressure(int pointer, int index)
        {
            return getPressure();
        }

        public float getHistoricalPressure(int index)
        {
            return getPressure();
        }

        public long getHistoricalEventTime(int index)
        {
            return getEventTime();
        }

        public float getHistoricalX(int index)
        {
            return getX();
        }

        public float getHistoricalY(int index)
        {
            return getY();
        }

        public float getHistoricalX(int pointer, int index)
        {
            return getX(pointer);
        }

        public float getHistoricalY(int pointer, int index)
        {
            return getY(pointer);
        }
    }
    ...
    public static class MotionEvent
    {
        ...
        // public final int ACTION_POINTER_UP = 3;
        // public final int ACTION_POINTER_INDEX_SHIFT = 4;
        ...
        public static MotionEvent create(int action, int x, int y, int pointer,
                int data, long nanoTime)
        {
